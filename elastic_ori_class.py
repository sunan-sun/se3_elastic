...
class elastic_ori_class:
    def __init__(self, Prior_list, Mu_arr, Sigma_arr, first_joint, last_joint) -> None:
        ...
        for k in range(K):
            ...
            Mu_red = quat_tools.riem_log(last_joint, Mu_arr[k, :])[0, :] # 2d array to 1d
            Mu_k, _, _, _  = np.linalg.lstsq(normal_basis, Mu_red, rcond=None)
            Mu[k, :] = Mu_k
            Sigma[k, :, :] = normal_basis.T @ Sigma_arr[k, :, :] @ normal_basis
    ...
    def map(self, q):
        ...
        if isinstance(q, R):
            q1 = quat_tools.riem_log(self.att, q.as_quat())[0, :] #2d array to 1d
        elif isinstance(q, np.ndarray):
            q1 = quat_tools.riem_log(self.att, q)[0, :] #2d array to 1d
            ...
            q1 = quat_tools.riem_log(R.from_quat(self.att), q).T

        v1, _, _, _ = np.linalg.lstsq(self.normal_basis, q1, rcond=None)
    ...
    def start_adapting(self):
        ...
        for i in range(M):
            ...
            ori_i = self.normal_basis @ ori_i_red
            ori_i_quat = quat_tools.riem_exp(self.att, ori_i.reshape(1, -1))
            new_ori[i] = R.from_quat(ori_i_quat[0])
        ...
        for k in range(K_ori):
            mu_k = mean_arr[k, :]
            mu_k = self.normal_basis @ mu_k
            # mu_k = self.normal_basis @ mu_k + self.normal_vec
            mu_k = quat_tools.riem_exp(self.att, mu_k.reshape(1, -1))
            Mu[k] = R.from_quat(mu_k[0])

        Sigma = np.zeros((K_ori, 4, 4))
        for k in range(K_ori):
            Sigma[k, :, :] = self.normal_basis @ cov_arr[k, :, :] @ self.normal_basis.T
