...
def start_adapting(traj, gmm, target_start_pose, target_end_pose, dt=None, scale_ratio=None):
    ...
    # anchor_arr = _get_joints(mu, sigma, end_point, start_point)[::-1]  # reverse anchor from the beginning to the end
    # pi = pi[::-1]    # reverse mu, sigma, and pi, go from the beginning to the end
    # mu = mu[::-1]
    # sigma = sigma[::-1]

    anchor_arr = _get_joints(mu, sigma, start_point, end_point) # anchor from the beginning to the end
